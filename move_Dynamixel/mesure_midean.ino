...
void measure_distance(float &distance1, float &distance2, float &distance3) {
    distance1 = lidarLite.distance();  // 첫 번째 측정
    distance2 = lidarLite.distance();  // 두 번째 측정
    distance3 = lidarLite.distance();  // 세 번째 측정
}

void move() {
  for (int tilt = tilt_start; tilt <= tilt_fin; tilt += 32) { 
    ...
    for (int pan = pan_start; pan <= pan_fin; pan += step) {
      ...
      measure_distance(distance1, distance2, distance3);
